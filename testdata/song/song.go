...
// FetchSongByPKs returns a row from song table with given primary key value
func FetchSongByPK(val uint) *Song {
	return coredb.FetchByPK[Song](val, "id", _db)
}

// FetchByPKs returns a row with selected fields from song table with given primary key value
func FetchByPK[T any](val uint) *T {
	return coredb.FetchByPK[T](val, "id", _db)
}

// FetchSongByPKs returns rows with from song table with given primary key values
func FetchSongByPKs(vals ...uint) []*Song {
	pks := coredb.GetAnySlice(vals)
	return coredb.FetchByPKs[Song](pks, "id", _db)
}

// FetchByPKs returns rows with selected fields from song table with given primary key values
func FetchByPKs[T any](vals ...uint) []*T {
	pks := coredb.GetAnySlice(vals)
	return coredb.FetchByPKs[T](pks, "id", _db)
}

// FindOneSong returns a row from song table with arbitary where query
...
func Count(whereSQL string, params ...any) (int, error) {
	return coredb.QueryInt("SELECT COUNT(*) FROM song "+whereSQL, _db, params...)
}

// FindOne returns a row with selected fields from song table with arbitary where query
// whereSQL must start with "where ..."
...
// FindSong returns rows from song table with arbitary where query
// whereSQL must start with "where ..."
...
// Find returns rows with selected fields from song table with arbitary where query
// whereSQL must start with "where ..."
...
func (c *Song) Insert() error {
	sql := `INSERT INTO song (title, hash) values (?, ?)`
