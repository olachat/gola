...
// SongUserFavourite represents song_user_favourite table
...
func (*SongUserFavouriteTable) GetTableName() string {
	return "song_user_favourite"
}

var table *SongUserFavouriteTable

// FetchSongUserFavouriteByPKs returns a row from song_user_favourite table with given primary key value
func FetchSongUserFavouriteByPK(val uint) *SongUserFavourite {
	return coredb.FetchByPK[SongUserFavourite](val, "user_id", _db)
}

// FetchByPKs returns a row with selected fields from song_user_favourite table with given primary key value
func FetchByPK[T any](val uint) *T {
	return coredb.FetchByPK[T](val, "user_id", _db)
}

// FetchSongUserFavouriteByPKs returns rows with from song_user_favourite table with given primary key values
...
// FetchByPKs returns rows with selected fields from song_user_favourite table with given primary key values
func FetchByPKs[T any](vals ...uint) []*T {
	pks := coredb.GetAnySlice(vals)
	return coredb.FetchByPKs[T](pks, "user_id", _db)
}

// FindOneSongUserFavourite returns a row from song_user_favourite table with arbitary where query
...
func Count(whereSQL string, params ...any) (int, error) {
	return coredb.QueryInt("SELECT COUNT(*) FROM song_user_favourite "+whereSQL, _db, params...)
}

// FindOne returns a row with selected fields from song_user_favourite table with arbitary where query
// whereSQL must start with "where ..."
...
// FindSongUserFavourite returns rows from song_user_favourite table with arbitary where query
// whereSQL must start with "where ..."
...
// Find returns rows with selected fields from song_user_favourite table with arbitary where query
// whereSQL must start with "where ..."
...
func (c *SongUserFavourite) Insert() error {
	sql := `INSERT INTO song_user_favourite (song_id, is_favourite, created_at, updated_at) values (?, ?, ?, ?)`
...
func (c *SongUserFavourite) Update() (bool, error) {
	...
	sql := `UPDATE song_user_favourite SET `
...
func (c *SongUserFavourite) Delete() error {
	sql := `DELETE FROM song_user_favourite WHERE user_id = ?`
