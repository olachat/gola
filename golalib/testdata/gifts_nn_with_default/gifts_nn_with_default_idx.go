...
func (q *idxQuery[T]) OrderBy(args ...orderBy) coredb.ReadQuery[T] {
	...
	for i, arg := range args {
		...
		case UpdateTime2Asc:
			q.orders[i] = "`update_time2` asc"
		case UpdateTime2Desc:
			q.orders[i] = "`update_time2` desc"
		case BranchesAsc:
...
// Select returns rows from `gifts_nn_with_default` table with index awared query
func Select() iQuery[GiftsNnWithDefault] {
	return new(idxQuery[GiftsNnWithDefault])
}

// SelectFields returns rows with selected fields from `gifts_nn_with_default` table with index awared query
