...
// Select returns rows from `gifts_with_default` table with index awared query
func Select() iQuery[GiftsWithDefault] {
	return new(idxQuery[GiftsWithDefault])
}

// SelectFields returns rows with selected fields from `gifts_with_default` table with index awared query
