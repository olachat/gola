...
// Select returns rows from `gifts_nn` table with index awared query
func Select() iQuery[GiftsNn] {
	return new(idxQuery[GiftsNn])
}

// SelectFields returns rows with selected fields from `gifts_nn` table with index awared query
