...
import (
	...
	"github.com/olachat/gola/golalib/testdata/gifts_nn_with_default"
)
...
func TestFetchGiftNotNullWithDefault2(t *testing.T) {
	gift := gifts_nn_with_default.FetchByPK(2)
	if gift == nil {
		t.Fatal("gift nn should not be nil")
	}
	if gift.GetId() != 2 {
	...
	if gift.GetGiftType() != gifts_nn_with_default.GiftsNnWithDefaultGiftTypeFreebie {
		t.Error("wrong gift type")
	}

	if gift.GetIsFree() != false {
	...
	ti, err := time.Parse("2006-01-02 15:04:05", "2019-01-01 00:00:01")
	if err != nil {
		panic(err)
	}
